...
async function fetchNextCard() {
    ...
    const res = await fetch(`${API_URL}/get-due-card?course=default`);
    const data = await res.json();
    if (!res.ok || data.message) {
        questionText.textContent = data.message || 'No cards available.';
        return;
    }
    currentCard = data;
...
function displayCard(card) {
    ...
    if (card.explanation) {
        ...
        expl.innerHTML = `<strong>Explanation:</strong> ${card.explanation}`;
        answerText.appendChild(expl);
    }
}
